...
define('THEME_SLUG', str_replace(" ", "_", strtolower(THEME_NAME)));
...
function nimbus_register_sidebars() {
    ...
    while ($i <= 20) {
        register_sidebar(array(
            ...
            'id' => 'sidebar_' . $i,
            'description' => __('Widgets in this area will be displayed in the sidebar for any post, or page items that are taged with sidebar', 'nimbus_opulus_sombre') . $i . '.',
            'before_widget' => '<div class="sidebar_widget sidebar sidebar_editable widget %2$s">',
            'after_widget' => '</div>',
            'before_title' => '<h3 class="widgettitle">',
            'after_title' => '</h3>'
...
/* **************************************************************************************************** */
    ...
    ?>  
        ...
        <p><span style="color:#fc7e2a;">This feature is availible to Nimbus Themes members.</span> <a href="http://www.nimbusthemes.com/?utm_source=<?php echo THEME_SLUG; ?>&utm_medium=theme&utm_content=seo_meta_link&utm_campaign=<?php echo THEME_SLUG; ?>">Join today!!</a></p>
...
function nimbus_call_banner_meta_box($post) {
    ...
    if (get_post_meta($post->ID, 'frontpage_banner_content', true)) {
        $value = get_post_meta($post->ID, 'frontpage_banner_content', true);
    } else {
        $value[0] = '';
    }
