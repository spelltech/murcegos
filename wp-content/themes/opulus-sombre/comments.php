...
<?php } else { ?>
    ...
    <?php if ('open' == $post->comment_status) { ?>
            <p><?php _e('Seja o primeiro a comentar.', 'nimbus_opulus_sombre'); ?></p>
...
if ('open' == $post->comment_status) {
    ...
    $fields = array(
        'author' => '<p class="comment-form-author"><label for="author">' . __('Nome', 'nimbus_opulus_sombre') . '</label> ' . ( $req ? '<span class="required">*</span>' : '' ) . '<input id="author" name="author" type="text" value="' . esc_attr($commenter['comment_author']) . '" size="30"' . $aria_req . ' /></p>',
    ...
    $modified_defaults = array(
        ...
        'comment_field' => '<div class="span5"><p><label for="comment">Comentário</label><br /><textarea id="comment" name="comment" cols="45" rows="8" aria-required="true"></textarea></p></div></div>',
        'must_log_in' => '<p class="must-log-in">' . sprintf(__('Você precisa estar <a href="%s">conectado</a> para comentar.'), wp_login_url(apply_filters('the_permalink', get_permalink()))) . '</p>',
        'logged_in_as' => '<div class="row"><div class="span3"><p class="logged-in-as">' . sprintf(__('Conectado como <a href="%1$s">%2$s</a>. <a href="%3$s" title="Logout">Sair?</a>'), admin_url('profile.php'), $user_identity, wp_logout_url(apply_filters('the_permalink', get_permalink()))) . '</p></div>',
        'comment_notes_before' => '',
        'comment_notes_after' => '<p class="form_allowed_tags">' . sprintf(__('Você pode usar <abbr title="HyperText Markup Language">HTML</abbr> tags e atributos: %s'), ' <code>' . allowed_tags() . '</code>') . '</p>',
        'id_form' => 'commentform',
        'id_submit' => 'submit',
